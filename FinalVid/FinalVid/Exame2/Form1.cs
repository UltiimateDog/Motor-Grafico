...
{   
    ...
    public partial class Form1 : Form
    {
        ...
        {
            caso = 1;
            LBL_Status.Text = "Rotating Y";
        }

        private void ButtonZ_Click(object sender, EventArgs e)
        {
            caso = 2;
            LBL_Status.Text = "Rotating Z";
        }
        ...
        private void ConvolutionBtn_Click(object sender, EventArgs e)
        {
            if (!convolucion)
            {
                ...
                {
                    Bitmap bitmap = new Bitmap(pictureBox1.Image);  // Use the existing image in pictureBox1
                    pictureBox1.Image = Canvas.ConvolucionOptimized(bitmap, bitmap.Width, bitmap.Height);
                    pictureBox1.Refresh();  // Refresh the PictureBox to update the display
                    convolucion = true;  // Set the flag to true indicating convolution is applied
                    if (timer1.Enabled)
                    {
                        timer1.Stop();  // Stop the timer if it's running to prevent it from redrawing the original image
                    }
                }
            }
            else
            {
                if (canvas != null && canvas.bitmap != null)
                {
                    pictureBox1.Image = new Bitmap(canvas.bitmap);  // Reset to original image
                    pictureBox1.Refresh();  // Refresh the PictureBox
                    convolucion = false;  // Reset the flag
                    timer1.Start();  // Restart the timer if needed for other updates
                }
            }
        }
